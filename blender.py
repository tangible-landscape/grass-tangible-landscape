...
def blender_export_vector(vector, path, vtype, name=None, z=False, tmp_path='/tmp', time_suffix=False, env=None):
    ...
    if os.path.exists(out):
        _removeShapefile(path if local else tmp_path, '{name}{suffix}.*'.format(name=name, suffix=suffix))
    try:
        params = {}
        if vtype == 'line':
            params['lco'] = 'SHPT=ARC'
    ...
    if not local:
        for each in glob.glob(os.path.join(tmp_path, '{name}{suffix}.*'.format(name=name, suffix=suffix))):
            ext = each.split('.')[-1]
            try:
                shutil.copyfile(each, os.path.join(path, '{name}{suffix}.{ext}'.format(name=name, suffix=suffix, ext=ext)))
            except OSError as e:
                if e.errno == 95:
                    pass
