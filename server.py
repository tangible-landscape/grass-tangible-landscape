...
def run_model(settings, steering):
    ...
    env = get_environment(
        n=region[0], s=region[1], w=region[2], e=region[3], align=region[4]
    )
    gscript.run_command(
        "g.remove",
        flags="fe",
...
def clientInterface(conn, connections, event, steering):
    ...
    while True:
        ...
        if message[0] == "clientfile":
            ...
            f.close()
            gscript.run_command(
                "r.unpack", input=server_path, output=name, overwrite=True
            )
        ...
        if message[0] == b"cmd":
            ...
            if "computation" not in connections:
                ...
                thread = Thread(
                    target=check_baseline,
                    args=(
                        connections["interface"],
                        params["probability_series"],
                        sod_process,
                        event,
                    ),
                )
...
def check_output(connection, basename, sod_process, event):
    ...
    while is_running:
        ...
        found = gscript.list_grouped(
            type="raster", flag="e", pattern=basename + "_[0-9]{4}_[0-9]{2}_[0-9]{2}"
        ...
        for each in found:
            if each not in old_found:
                ...
                gscript.run_command(
                    "r.pack", input=each, output=pack_path, overwrite=True, quiet=True
                )
                event.clear()
                connection.sendall(
                    "serverfile:{}:{}".format(
                        os.path.getsize(pack_path), pack_path
                    ).encode()
                )
                old_found.append(each)
                _debug(
                    _debug_file,
                    "serverfile:{}:{}".format(os.path.getsize(pack_path), pack_path)
                    + "\n",
                )
        if not is_running:
            event.wait(5)
...
def check_baseline(connection, basename, sod_process, event):
    ...
    for each in found:
        ...
        event.clear()
        connection.sendall(
            "serverfile:{}:{}".format(os.path.getsize(pack_path), pack_path).encode()
        )
    event.wait(5)
    ...
    while True:
        ...
        for m in message:
            ...
            if lm[0] == b"output":
                ...
                gscript.run_command(
                    "r.pack", input=lm[1], output=pack_path, overwrite=True, quiet=True
                )
                if "interface" in connections:
                    event.clear()
                    connections["interface"].sendall(
                        "serverfile:{}:{}".format(
                            os.path.getsize(pack_path), pack_path
                        ).encode()
                    )
...
if __name__ == "__main__":
    if len(sys.argv) == 4:
        ...
        print(
            "Incorrect number of arguments, is {a1}, should be {a2}".format(
                a1=len(sys.argv) - 1, a2=" 2 or 3"
            )
        )
        sys.exit(1)
