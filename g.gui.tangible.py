...
class AboutPanel(wx.Panel):
    def __init__(self, parent, scaniface):
        ...
        url = UrlCtrl(
            self,
            label="tangible-landscape.github.io",
            url="tangible-landscape.github.io",
        )
        wiki = UrlCtrl(
            self,
            label="See documentation and tutorial on wiki",
...
class AnalysesPanel(wx.Panel):
    def __init__(self, parent, giface, settings, scaniface):
        ...
        if (
            "contours" in self.settings["analyses"]
            and self.settings["analyses"]["contours"]
        ):
        ...
        self.addContours = wx.BitmapButton(
        ...
        sizer.Add(
            wx.StaticText(self, label="Contour map name:"),
            proportion=0,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=5,
        )
        sizer.Add(
            self.contoursSelect, proportion=4, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        sizer.Add(
            self.addContours,
            proportion=0,
        ...
        sizer.Add(
            wx.StaticText(self, label="Interval:"),
            proportion=0,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=5,
        )
        sizer.Add(
            self.contoursStepTextCtrl,
            proportion=1,
        ...
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            wx.StaticText(self, label="Raster with training areas:"),
            proportion=0,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=5,
        )
    ...
    def OnAnalysesChange(self, event):
        ...
        self.settings["analyses"][
            "contours_step"
        ] = self.contoursStepTextCtrl.GetValue()
...
class ScanningPanel(wx.Panel):
    def __init__(self, parent, giface, settings, scaniface):
        ...
        self.elevInput.SetToolTip(
            "Raster from which we take the georeferencing information"
        )
        ...
        if self.scaniface.sensor == "k4a":
            ...
            self.cameraResolution.SetToolTip(
                "Applicable only when outputting color raster.\n"
            ...
            self.whiteBalance = SpinCtrl(self, min=0, max=6500, initial=0)
        ...
        hSizer.Add(
            wx.StaticText(self, label="Rotation angle:"),
            proportion=1,
        ...
        hSizer = wx.BoxSizer(wx.HORIZONTAL)
        hSizer.Add(
            wx.StaticText(self, label="Trim vertically [cm]:"),
            proportion=1,
            flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
            border=5,
        )
        for each in "tb":
            hSizer.Add(
                wx.StaticText(self, label=each.upper() + ":"),
                flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
                border=2,
            )
            hSizer.Add(
                self.trim[each], flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL, border=2
            )
        hSizer.Add(
            self.btnCalibrateTilt, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL, border=2
        )
        geomSizer.Add(hSizer, flag=wx.EXPAND)
        hSizer = wx.BoxSizer(wx.HORIZONTAL)
        hSizer.Add(
            wx.StaticText(self, label="Trim horizontally [cm]:"),
            proportion=1,
            flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
            border=5,
        )
        ...
        hSizer.Add(
            self.btnCalibrateExtent, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL, border=2
        )
        geomSizer.Add(hSizer, flag=wx.EXPAND)
        # automatic trim
        hSizer = wx.BoxSizer(wx.HORIZONTAL)
        hSizer.Add(
            wx.StaticText(self, label="Trim tolerance [0-1]:"),
            proportion=1,
            flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
            border=5,
        )
        hSizer.Add(
            self.trim_tolerance, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL, border=5
        )
        geomSizer.Add(hSizer, flag=wx.EXPAND)
        ...
        if self.scaniface.sensor != "k4a":
            ...
            hSizer = wx.BoxSizer(wx.HORIZONTAL)
            hSizer.Add(
                wx.StaticText(self, label="Reference DEM:"),
                flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
                border=5,
            )
            hSizer.Add(
            ...
            hSizer = wx.BoxSizer(wx.HORIZONTAL)
            hSizer.Add(
                wx.StaticText(self, label="Reference region:"),
                flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
                border=5,
            )
            hSizer.Add(
            ...
            hSizer = wx.BoxSizer(wx.HORIZONTAL)
            hSizer.Add(
                wx.StaticText(self, label="Z-exaggeration:"),
                proportion=1,
                flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
                border=5,
            )
        ...
        hSizer = wx.BoxSizer(wx.HORIZONTAL)
        hSizer.Add(
            wx.StaticText(self, label="Number of scans:"),
            proportion=1,
            flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
            border=5,
        )
        ...
        hSizer = wx.BoxSizer(wx.HORIZONTAL)
        hSizer.Add(
            wx.StaticText(self, label="Smooth value:"),
            proportion=1,
            flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
            border=5,
        )
        ...
        hSizer = wx.BoxSizer(wx.HORIZONTAL)
        hSizer.Add(
            wx.StaticText(self, label="Resolution [mm]:"),
            proportion=1,
            flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
            border=5,
        )
        ...
        if self.scaniface.sensor == "k4a":
            ...
            # region
            hSizer.Add(
                wx.StaticText(self, label="or region:"),
                proportion=0,
                flag=wx.RIGHT | wx.ALIGN_CENTER_VERTICAL,
                border=5,
            )
            ...
            hSizer.Add(
                self.zexag, proportion=0, flag=wx.ALIGN_CENTER_VERTICAL, border=5
            )
            georefSizer.Add(hSizer, proportion=1, flag=wx.EXPAND | wx.ALL, border=5)
            mainSizer.Add(
                georefSizer, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, border=10
            )
...
class TangibleLandscapePlugin(wx.Dialog):
    def __init__(self, giface, parent):
        ...
        if not self.giface.GetLayerTree():
            self.giface.WriteWarning(
                (
                    "Running in standalone mode with "
        ...
        if "tangible" not in self.settings:
            self.settings["tangible"] = {
                "calibration": {"matrix": None},
                "analyses": {"file": None, "contours": None, "contours_step": 1},
            }
        ...
        self.scanning_panel = ScanningPanel(
            self.notebook, self.giface, self.settings["tangible"], scaniface=self
        )
        self.notebook.AddPage(self.scanning_panel, "Scanning")
        self.scan = self.settings["tangible"]["scan"]

        self.outputPanel = OutputPanel(
            self.notebook, self.giface, self.settings["tangible"]
        )
        self.notebook.AddPage(self.outputPanel, "Output")
        self.scanning_panel.settingsChanged.connect(
            lambda: setattr(self, "changedInput", True)
        )
        analyses_panel = AnalysesPanel(
            self.notebook, self.giface, self.settings["tangible"], scaniface=self
        )
        self.notebook.AddPage(analyses_panel, "Analyses")
        analyses_panel.settingsChanged.connect(
            lambda: setattr(self, "changedInput", True)
        )
        self.outputPanel.settingsChanged.connect(
            lambda: setattr(self, "changedInput", True)
        )
        ...
        self.activities_panel = ActivitiesPanel(
            self.notebook, self.giface, self.settings["tangible"], scaniface=self
        )
    ...
    def Calibrate(self, event):
        ...
        dlg = wx.MessageDialog(
            self,
            "In order to calibrate, please remove objects from the table.",
            "Calibration",
        ...
        if not (res["calib_matrix"] and len(res["calib_matrix"].split(",")) == 9):
            ...
            self.giface.WriteCmdLog(
                "Measured and corrected tilting of sensor: {angle} degrees".format(
                    angle=res["angle_deviation"]
                )
            )
            if float(res["angle_deviation"]) > 3:
                self.giface.WriteWarning(
                    "Angle deviation is too high, please level the sensor."
                )
    ...
    def GatherParameters(self, editMode, continuous):
        ...
        params = {}
        if (
            self.settings["tangible"]["output"]["calibrate"]
            and self.settings["tangible"]["output"]["calibration_scan"]
        ):
            params["output"] = (
                self.settings["tangible"]["output"]["calibration_scan"] + "tmp"
            )
        elif self.settings["tangible"]["output"]["scan"]:
            params["output"] = self.settings["tangible"]["output"]["scan"] + "tmp"
        # drawing
        if (
            self.settings["tangible"]["drawing"]["active"]
            and self.settings["tangible"]["drawing"]["name"]
        ):
        ...
        if self.settings["tangible"]["analyses"]["contours"] and "output" in params:
            ...
            params["contours_step"] = self.settings["tangible"]["analyses"][
                "contours_step"
            ]
        elif editMode:
            params["contours"] = ""
        ...
        if (
            "output" in self.settings["tangible"]
            and self.settings["tangible"]["output"]["PLY"]
            and self.settings["tangible"]["output"]["PLY_file"]
            and not self.settings["tangible"]["drawing"]["active"]
        ):
    ...
    def Scan(self, continuous):
        ...
        self.process = gscript.start_command(
            "r.in.kinect",
            overwrite=True,
            quiet=True,
            stdin=PIPE,
            signal_file=self._getSignalFile(),
            **params
        )
        return self.process
    ...
    def Start(self):
        ...
        handlers = [
            SignalFileChangeHandler(
                self.runImport, os.path.basename(self._getSignalFile())
            ),
            DrawingChangeHandler(
                self.runImportDrawing, self.settings["tangible"]["drawing"]["name"]
            ),
        ]
    ...
    def OnCheckLayers(self, event):
        ...
        if not hasattr(ll, "CheckLayer"):
            print(
                "Checking and unchecking layers in layer Manager requires GRASS GIS version > 7.2"
            )
            return
        for each in event.layers:
