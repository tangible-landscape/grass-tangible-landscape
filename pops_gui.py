...
class PopsPanel(wx.Panel):
    def __init__(self, parent, giface, settings, scaniface):
        ...
        self.empty_placeholders = {
            "results": "results_tmp",
            "treatments": "treatments_tmp",
        }
        ...
        # config file
        self.configFileCtrl = filebrowse.FileBrowseButton(
            self, labelText="Configuration:", changeCallback=self._loadConfiguration
        )
        ...
        self.visualizationChoice = wx.Choice(
            modelingBox, choices=self.visualizationModes
        )
        ...
        self.replaySelect = Select(
            modelingBox, size=(-1, -1), type="raster", fullyQualified=False
        )
        ...
        sizer.Add(
            self.configFileCtrl, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        ...
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            runBtn, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, border=5
        )
        sizer.Add(
            self.visualizationChoice,
            proportion=1,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=5,
        )
        ...
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            wx.StaticText(modelingBox, label="Treatment area:"),
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=5,
        )
        sizer.Add(
            self.treatmentSelect,
            proportion=1,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=5,
        )
        ...
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            wx.StaticText(modelingBox, label="Replay scenario:"),
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=5,
        )
        sizer.Add(
    ...
    def _connect(self):
        ...
        if (
            "dashboard" in self.configuration["POPS"]
            and self.configuration["POPS"]["dashboard"]
        ):
    ...
    def _connectSteering(self):
        ...
        self.steeringClient = SteeringClient(
            urlS,
            port_interface=steering_dict["port_interface"],
            port_simulation=port_simulation,
            launch_server=server,
            local_gdbase=local_gdbase,
            log=self.giface,
            eventHandler=self,
        )
    ...
    def OnDisplayUpdate(self, event):
        ...
        if self.showDisplayChange:
            cumulativeArea = (
                sum(self.treatmentHistory[: self.currentRealityCheckpoint]) + event.area
            )
            ...
            if self.steeringClient.is_steering():
                self.dashboardFrame.show_value(
                    [
                        event.area / coef,
                ...
                self.dashboardFrame.show_value(
                    [event.area / coef, event.area * cost / money_coef]
                )

    def OnTimeDisplayUpdate(self, event):
        if self.timeDisplay:
            self.timeDisplay.UpdateText(
                event.current,
                event.currentView,
                self.visualizationModes[self.visualizationMode],
            )
        if self.timeStatusDisplay:
            self.timeStatusDisplay.UpdateText(
                event.currentView,
                "forecast" if self._ifShowProbability() else "occurrence",
            )
    ...
    def get_current_year(self):
        if self.steeringClient.is_steering():
            if self.params.pops["steering"]["move_current_year"]:
                return (
                    dateFromString(self.params.model["start_date"]).year
                    + self.currentCheckpoint
                )
            return (
                dateFromString(self.params.model["start_date"]).year
                + self.currentRealityCheckpoint
            )
    ...
    def _uploadStepToDashboard(self, event):
        ...
        if res:
            ...
            average_infected = "__".join(
                [self.params.model["average_series"], single_infected]
            )
            stddev_infected = "__".join(
                [self.params.model["stddev_series"], single_infected]
            )
            ...
            if self.webDashboard:
                checkpoint = (
                    int(year) - dateFromString(self.params.model["start_date"]).year + 1
                )
                ...
                self.webDashboard.upload_results(
                    year,
                    event.name,
                    single_infected,
                    average_infected,
    ...
    def _computeDifference(self, names):
        ...
        for each in names:
            ...
            try:
                gscript.mapcalc(
                    "{n} = {c} - {p}".format(n=new_name, c=each, p=previous), env=env
                )
    ...
    def _renameAllAfterSimulation(self, name):
        ...
        pattern_layers = gscript.list_grouped(type="raster", pattern=pattern, flag="e")[
        ...
        if pattern_layers:
            for layer in pattern_layers:
                ...
                new_name = "__".join(
                    components[:-1]
                    + ["{a1}_{a2}".format(a1=a1, a2=a2)]
                    + components[-1:]
                )
                gscript.run_command(
                    "g.copy", raster=[layer, new_name], quiet=True, overwrite=True
                )
    ...
    def _ifShowProbability(self, evalFuture=False):
        ...
        if self.visualizationModes[self.visualizationMode] == "combined":
            if (
                self.currentCheckpoint + (1 if evalFuture else 0)
                > self.currentRealityCheckpoint
            ):
                return True
            return False
    ...
    def ResetSimulation(self, event):
        ...
        if self.webDashboard:
            ...
            callback = partial(
                self.treatments.current_geojson_to_treatment,
                year=self.get_current_year(),
            )
    ...
    def Replay(self, scenario):
        ...
        if not f["fullname"]:
            ...
            if name.startswith(self.params.model["probability_series"]):
                rules = os.path.join(
                    self.workdir, self.configuration["POPS"]["color_probability"]
                )
            else:
                rules = os.path.join(
                    self.workdir, self.configuration["POPS"]["color_trees"]
                )
            ...
            try:
                ...
                gscript.run_command(
                    "r.colors", map=name, quiet=True, rules=rules, flags="w", env=env
                )
            except CalledModuleError:
                pass
            ...
            opacity = (
                float(self.params.pops["results_opacity"])
                if "results_opacity" in self.params.pops
                else 1
            )
            self._changeResultsLayer(
                cmd=cmd,
                name=name,
                opacity=opacity,
                resultType="results",
    ...
    def ShowBaseline(self):
        ...
        if not f["fullname"]:
            # display empty raster
            self._changeResultsLayer(
                cmd=["d.rast", "map=" + self.empty_placeholders["results"]],
                name=self.empty_placeholders["results"],
                resultType="results",
                useEvent=False,
            )
        else:
    ...
    def ShowResults(self):
        ...
        prefix_prob = (
            self.configuration["POPS"]["model"]["probability_series"] + "__" + prefix
        )
        suffix = "{y}_{m:02d}_{d:02d}".format(
            y=displayTime[0], m=displayTime[1], d=displayTime[2]
        )
    ...
    def ShowTreatment(self):
        ...
        name = self._createPlayerName()
        name = "__".join(
            [self.configuration["POPS"]["treatments"], event, name, attempt]
        )
        style = {
            "fill_color": "none",
            "width": 2,
        ...
        self._changeResultsLayer(
            cmd=cmd, name=name, resultType="treatments", useEvent=False
        )
    ...
    def ShowTreatmentReplay(self, name, checkpoint):
        ...
        name = "__".join(
            [
                self.configuration["POPS"]["treatments"],
                event,
                player,
                attempt.split("_")[0],
            ]
    ...
    def RunBaseline(self, event):
        ...
        self.steeringClient.baseline_set_params(
            self.params.model_name, baseline_params, self.params.model_flags, region
        )
    ...
    def _RunSimulation(self, event=None):
        ...
        if self.switchCurrentResult == 0:
            ...
            self.HideResultsLayers()
            wx.CallLater(
                self.configuration["POPS"]["waitBeforeRun"], self.RunSimulation
            )
    ...
    def _initSimulation(self, restart):
        ...
        self.steeringClient.simulation_set_params(
            self.params.model_name, model_params, self.params.model_flags, region
        )
    ...
    def RunSimulation(self, event=None):
        ...
        # save treatment
        tr_name = self.treatments.archive_treatments(
            event, playerName, new_attempt, checkpoint, bool(self.webDashboard)
        )
        # measuring area
        self.treated_area = self.treatments.compute_registered_treatment_area(
            bool(self.webDashboard)
        )
        ...
        self.treatments.create_treatment_visualization_vector(
            event, playerName, new_attempt, checkpoint
        )

        tr_year = self.get_current_year()
        if self.webDashboard:
            self.webDashboard.set_management(
                geojson=self.treatments.tr_dashboard_json,
                cost=self.money_spent,
                area=self.treated_area,
                year=tr_year,
            )
        ...
        for each in self.treatments.treatments_for_model:
            ...
            self.steeringClient.simulation_load_data(
                tr_name,
                each["treatment_date"],
                each["treatment_length"],
                each["treatment_application"],
            )
        if self.params.pops["steering"]["move_current_year"]:
        ...
        if not self.steeringClient.results_empty():
            ...
            while not found and not self.steeringClient.results_empty():
                ...
                if res:  # should happen always?
                    ...
                    self.checkpoints[currentCheckpoint] = (
                        int(year),
                        int(month),
                        int(day),
                    )
    ...
    def StartTreatment(self):
        ...
        if "scanning_params" in self.configuration["tasks"][self.current]:
            for each in self.configuration["tasks"][self.current][
                "scanning_params"
            ].keys():
                self.settings["scan"][each] = self.configuration["tasks"][self.current][
                    "scanning_params"
                ][each]
        # resume scanning
        if "filter" in self.configuration["tasks"][self.current]:
            ...
            self.scaniface.filter["threshold"] = self.configuration["tasks"][
                self.current
            ]["filter"]["threshold"]
            self.scaniface.filter["debug"] = self.configuration["tasks"][self.current][
                "filter"
            ]["debug"]
        if (
            "single_scan" in self.configuration["tasks"][self.current]
            and self.configuration["tasks"][self.current]["single_scan"]
        ):
            self._stopScanning()
        else:
            self._startScanning()
    ...
    def _bindButtons(self):
        ...
        def _register_and_update():
            ...
            self.treatments.register_treatment(
                self.get_current_year(), bool(self.webDashboard)
            )
        ...
        bindings = {
            "simulate": self._RunSimulation,
            "visualization": lambda evt: self.SwitchVizMode(),
            "stepforward": self.StepForward,
    ...
    def Step(self, forward=True):
        ...
        start = 0
            ...
            dateFromString(self.params.model["end_date"]).year
            - dateFromString(self.params.model["start_date"]).year
            + 1
        )
        ...
        if not forward and self.currentCheckpoint <= start:
            return
        self.currentCheckpoint = (
            self.currentCheckpoint + 1 if forward else self.currentCheckpoint - 1
        )
        if self.timeDisplay:
            self.timeDisplay.UpdateText(
                self.currentRealityCheckpoint,
    ...
    def ChangeRegion(self):
        ...
        if not region:
            ...
            self.settings["scan"]["elevation"] = self.configuration["tasks"][
                self.current
            ]["base"]
    ...
    def AddTempLayers(self):
        ...
        if not f["fullname"]:
            gscript.run_command(
                "v.edit",
                tool="create",
                map=self.empty_placeholders["treatments"],
                env=env,
            )
        ...
        ll.AddLayer(
            "vector",
            name=self.empty_placeholders["treatments"],
            cmd=["d.vect", "map=" + self.empty_placeholders["treatments"]],
            checked=True,
        )
        style = {}
        ...
        ll.AddLayer(
            "vector",
            name=self.treatments.tr_external_name,
            cmd=["d.vect", "map=" + self.treatments.tr_external_name] + style,
            checked=True,
        )
    ...
    def RemoveLayers(self, etype="raster", pattern=None, layers=None):
        ...
        all_layers = []
        if pattern:
            pattern_layers = gscript.list_grouped(type=etype, pattern=pattern)[
                gscript.gisenv()["MAPSET"]
            ]
    ...
    def StartTimeDisplay(self):
        self.timeDisplay = SteeringDisplayFrame(
            ...
            fontsize=self.configuration["tasks"][self.current]["time_display"][
                "fontsize"
            ],
            vtype=self.visualizationModes[self.visualizationMode],
    ...
    def StartTimeStatusDisplay(self):
        self.timeStatusDisplay = CurrentViewDisplayFrame(
            self,
            start=dateFromString(self.params.model["start_date"]).year,
            end=dateFromString(self.params.model["end_date"]).year + 1,
            fontsize=self.configuration["tasks"][self.current]["time_status_display"][
                "fontsize"
            ],
            beginning_of_year=self.configuration["tasks"][self.current][
                "time_status_display"
            ]["beginning_of_year"],
            fgcolor=self.configuration["tasks"][self.current][
                "time_status_display"
            ].get("fgcolor", None),
            bgcolor=self.configuration["tasks"][self.current][
                "time_status_display"
            ].get("bgcolor", None),
        )
...
class Attempt(object):
    ...
    def initialize(self, event, player):
        pattern = "{e}__{n}__[0-9]+_[0-9]+__[0-9]{{4}}_[0-9]{{2}}_[0-9]{{2}}".format(
            n=player, e=event
        )
        pattern_layers = gscript.list_grouped(type="raster", pattern=pattern, flag="e")[
            gscript.gisenv()["MAPSET"]
